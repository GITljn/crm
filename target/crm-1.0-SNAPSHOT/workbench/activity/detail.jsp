...
<script type="text/javascript">
        ...
        $("#saveRemarkBtn").click(function () {
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        var e = data.activityRemark
                        var html = ""
                        html += '<div id="'+e.id+'" class="remarkDiv" style="height: 60px;">'
                        html += '<img title="zhangsan" src="image/user-thumbnail.png" style="width: 30px; height:30px;">'
                        html += '<div style="position: relative; top: -40px; left: 40px;" >'
                        html += '<h5 id="note'+e.id+'">'+e.noteContent+'</h5>'
                        html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${activity.name}</b> <small style="color: gray;"> '+(0 == e.editFlag ? e.createTime : e.editTime)+' 由 '+(0 == e.editFlag ? e.createBy : e.editBy)+'</small>'
	...
	function deleteRemark(id) {
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    // 不能使用showRemarkList()，因为使用的after追加元素
                    $("#" + id).remove()
                } else {
                    alert("删除失败")
                }
...
<body>
    ...
    <div class="modal fade" id="editActivityModal" role="dialog">
        ...
        <div class="modal-dialog" role="document" style="width: 85%;">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button id="updateBtn" type="button" class="btn btn-primary" onclick="updateActivity('${activity.id}')">更新</button>
                </div>
            </div>
        </div>
    </div>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 250px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="editActivity('${activity.id}')"><span class="glyphicon glyphicon-edit"></span> 编辑</button>
			<button type="button" class="btn btn-danger" onclick="deleteActivity('${activity.id}')"><span class="glyphicon glyphicon-minus"></span> 删除</button>
		</div>
	</div>
