...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#saveBtn").click(function () {
			$.ajax({
				...
				success: function (data) {
					if (data.flag) {
						var e = data.clueRemark
						var html = ""
						html += '<div id="'+e.id+'" class="remarkDiv" style="height: 60px;">'
						html += '<img title="zhangsan" src="image/user-thumbnail.png" style="width: 30px; height:30px;">'
						html += '<div style="position: relative; top: -40px; left: 40px;" >'
						html += '<h5 id="note'+e.id+'">'+e.noteContent+'</h5>'
						html += '<font color="gray">线索</font> <font color="gray">-</font> <b>'+e.owner+'-动力节点</b> <small style="color: gray;"> '+(0 == e.editFlag?e.createBy +' 由'+e.createTime:e.editBy +' 由'+e.editTime)+'</small>'
		...
		// 查询未关联的市场活动列表
		$("#searchActivityList").keydown(function (e) {
			if (13 == e.keyCode){
				$.ajax({
					url: "workbench/clue/searchActivityList.do",
					type: "get",
					data: {
						"id": "${clue.id}",
					...
					success: function (data) {
						...
						$.each(data, function (i, e) {
							html += '<tr>'
							html += '<td><input type="checkbox" name="searchedActivityDx" value="'+e.id+'" onclick="searchedActivityDx()"/></td>'
							html += '<td>'+e.name+'</td>'
						...
						$("#searchedActivityList").html(html)
					}
				})
			}
		})
		...
		// 全选和反选未关联的市场活动列表
		$("#searchedActivityQx").click(function () {
			$("input[name=searchedActivityDx]").prop("checked", this.checked)
		})
		...
		$("#bindBtn").click(function () {
			...
			var checked = $("input[name=searchedActivityDx]:checked")
		...
		$.ajax({
			...
			success: function (data) {
				...
				$.each(data, function (i, e) {
					...
					html += '<td>'+e.endDate+'</td>'
					html += '<td>'+e.owner+'</td>'
					html += '<td><a href="javascript:void(0);" style="text-decoration: none;" onclick="unBind(\''+e.id+'\')"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>'
					html += '</tr>'
				})
	...
	function searchedActivityDx() {
		var len = $("input[name=searchedActivityDx]").length
		var lenChecked = $("input[name=searchedActivityDx]:checked").length
		if (lenChecked == len) {
			$("#searchedActivityQx").prop("checked", true)
		} else {
			$("#searchedActivityQx").prop("checked", false)
		}
	}
...
<body>
					...
					<table id="activityTable" class="table table-hover" style="width: 900px; position: relative;top: 10px;">
						<thead>
							<tr style="color: #B3B3B3;">
								<td><input id="searchedActivityQx" type="checkbox"/></td>
								<td>名称</td>
								<td>开始日期</td>
								<td>结束日期</td>
								<td>所有者</td>
	...
	<div id="remarkBody" style="position: relative; top: 40px; left: 40px;">
		<div id="remarkDivAfter" class="page-header">
			<h4>备注</h4>
		</div>
